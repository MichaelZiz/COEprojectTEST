...
public class Booked {
    ...
    public void SeatsAvailableDecremenet(int k) {//k is the number of seats to reserve
        //number of available seats after purchase of tickets
        int count = k;
        for (int i = 0; i < movies1.seats.length; i++) {
            if (movies1.seats[i] == 0 && count > 1) {
                movies1.seats[i] = 1;
                count--;

            } else {
                System.out.println("movie is full");// what if customer wants 4 seats but can only book 3 
            }
        }
